...
class Sketchpad(AbstractInput):
    def __init__(self, shape=(28, 28), invert_colors=True, flatten=False, label=None):
    ...
    def preprocess(self, inp):
        ...
        im = PIL.Image.new(
            "RGBA", im_transparent.size, "WHITE"
        ...
        if self.flatten:
            array = (
                np.array(im).flatten().reshape(1, self.image_width * self.image_height)
            )
        else:
            array = (
                np.array(im).flatten().reshape(1, self.image_width, self.image_height)
...
class Textbox(AbstractInput):
    def __init__(
        self, lines=1, placeholder=None, default=None, numeric=False, label=None
    ):
        self.lines = lines
    ...
    @classmethod
    def get_shortcut_implementations(cls):
        return {"text": {}, "textbox": {"lines": 7}, "number": {"numeric": True}}
...
class Radio(AbstractInput):
    ...
    def get_template_context(self):
        return {"choices": self.choices, **super().get_template_context()}
...
class Microphone(AbstractInput):
    ...
    def preprocess(self, inp):
        ...
        if self.preprocessing == "mfcc":
            return preprocessing_utils.generate_mfcc_features_from_audio_file(
                file_obj.name
            )
